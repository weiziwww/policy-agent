...
class SlackEventListener(View):
    def post(self, request, *args, **kwargs):
        ...
        if event.get("bot_id"):
            return JsonResponse({"status": "ignored bot event"})
        ...
        try:
            ...
            initiator = None
            if user_id:
                initiator = SlackUser.objects.filter(username=user_id, community=community).first()

            self.trigger_action(event_type, event, community, initiator)

        except SlackCommunity.DoesNotExist:
            logger.error(f"Received event for unknown Slack team: {team_id}")
            return JsonResponse({"status": "unknown team"}, status=400)
    ...
    def trigger_action(self, event_type, event, community, initiator):
        action_obj = None
        ...
        if event_type == "message":
            ...
            action_obj = SlackRenameConversation(
                community=community,
                initiator=initiator,
            ...
            action_obj = SlackJoinConversation(
                community=community,
                initiator=initiator,
                channel=event.get("channel", ""),
            ...
            if item.get("type") == "message":
                action_obj = SlackPinMessage(
                    community=community,
                    initiator=initiator,
                    channel=item.get("channel", ""),
                    timestamp=item.get("message", {}).get("ts", "")
                )

        if action_obj:
            logger.info(f"Triggering PolicyEngine for action: {action_obj.ACTION_NAME}")
            action_obj.save()
# import logging
# from django.shortcuts import redirect, render
# from django.contrib.auth import get_user
