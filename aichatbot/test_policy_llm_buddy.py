...
from policy_sandbox import build_test_suite, run_policy_check

def test_full_workflow_with_feedback_loop():
    ...
    for index, case in enumerate(test_cases, 1):
        print(f"\n==================================================")
        print(f" [Test Case {index}/{len(test_cases)}]: {case['name']}")
        print(f"==================================================")
        
        print("   >> Generating Initial Draft Code...")
        current_code = buddy.generate_code_with_context(case['description'], api_context_summary)
        
        print("   >> Static Verification...")
        current_code = buddy.verify_and_fix_code(current_code, case['description'], api_context_summary)
        
        for attempt in range(1, MAX_RETRIES + 1):
            print(f"   >> [Attempt {attempt}] Running Sandbox Tests...")
            
            suite = build_test_suite(case['name'])
            
            all_passed = True
            feedback_log = ""
            
            for scenario in suite:
                proposal, users, expected, detail = scenario["setup"]()
                actual_result = run_policy_check(current_code, proposal, users)
                
                if actual_result != expected:
                    all_passed = False
                    feedback_log += f"Scenario: {detail}\n"
                    feedback_log += f"Expected: '{expected}', but Got: '{actual_result}'\n\n"
            ...
            if all_passed:
                ...
                print(f"Tests failed. Generating logic fix...")
                if attempt < MAX_RETRIES:
                    current_code = buddy.fix_code_with_feedback(
                        generated_code=current_code,
                        execution_feedback=feedback_log,
                        user_description=case['description'],
                        api_context=api_context_summary
                    )
                else:
                    print(f"Max retries reached ({MAX_RETRIES}). Code still failing.")
                    print("   " + "-" * 40)
                    print(current_code)
                    print("   " + "-" * 40)

if __name__ == "__main__":
    test_full_workflow_with_feedback_loop()
# import json
# import os
# from prompts import PolicyLLMBuddy
